-- 15-min server cron for guest calendar sync — mirrors cron_sync_calendars.
...
create or replace function public.cron_sync_guest_calendars()
returns void
language plpgsql
security definer
...
begin
  ...
  if secret is null then raise notice 'cron_sync_guest_calendars: no cron_secret'; return; end if;
  perform net.http_post(
    url := 'https://xwuekcysigkujhyucugi.supabase.co/functions/v1/sync-guest-calendars',
...
do $$
begin
  perform cron.unschedule('sync-guest-calendars-15min')
  where exists (select 1 from cron.job where jobname = 'sync-guest-calendars-15min');
exception when others then null;
end $$;

select cron.schedule('sync-guest-calendars-15min', '*/15 * * * *', $cron$ select public.cron_sync_guest_calendars(); $cron$);
