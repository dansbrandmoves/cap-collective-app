...
// Cursor SVG used in the guide mockup
function GuideCursor({ className = '' }) {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="white" stroke="#1a1a1a" strokeWidth="1.4"
      className={className} style={{ filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.4))' }}>
...
// Shows the full Google warning flow in a single mockup: click Advanced → click "Go to coordie.com"
function GoogleOAuthGuide({ onConfirm, onCancel }) {
  ...
  useEffect(() => {
    const seq = [
      [300,  () => setPhase(1)],  // cursor starts flying to Advanced
      [2900, () => setPhase(2)],  // Advanced clicked → expand section
      [3400, () => setPhase(3)],  // cursor flies to "Go to" link
      [6000, () => setPhase(4)],  // "Go to" clicked
      [6800, () => setPhase(5)],  // done — CTA pulses
    ]
    const timers = seq.map(([delay, fn]) => setTimeout(fn, delay))
    return () => timers.forEach(clearTimeout)
  }, [])

  const expanded = phase >= 2  // show Advanced expanded section
  const TOTAL_MS = 6800

  // Which instruction to show
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-0 sm:px-4">
      ...
      <div className="relative w-full sm:max-w-[420px] bg-surface-900 border border-white/10 rounded-t-3xl sm:rounded-3xl shadow-sheet overflow-hidden animate-slideUp">
        ...
        {/* Single progress bar covering the whole sequence */}
        ...
        <div className="px-6 pt-5 pb-6">
          ...
          {/* Single Google screen mockup — stays consistent, content updates in place */}
          <div className="rounded-2xl overflow-hidden border border-white/8 shadow-xl mb-4">
            <div className="bg-white px-4 pt-4 pb-3">
              ...
              <div className="relative">
                {/* The "Advanced" toggle — always visible */}
                ...
                {/* Pulse ring on Advanced when cursor is heading there */}
                {phase >= 1 && phase < 2 && (
                  <div className="absolute -inset-0.5 rounded animate-pulse-ring pointer-events-none" />
                )}

                {/* Cursor flying to Advanced (phase 1) */}
                {phase === 1 && (
              ...
              {/* ── Expanded section — slides in after Advanced is clicked ── */}
              <div className={`overflow-hidden transition-all duration-500 ${expanded ? 'max-h-24 opacity-100 mt-1.5' : 'max-h-0 opacity-0'}`}>
                ...
                {/* Go to coordie.com (unsafe) — the final target */}
                <div className="relative inline-block">
                  ...
                  {/* Pulse ring on "Go to" when cursor is heading there */}
                  {phase >= 3 && phase < 5 && (
                    <div className="absolute -inset-0.5 rounded animate-pulse-ring pointer-events-none" />
                  )}

                  {/* Cursor flying DOWN to "Go to" (phase 3+) */}
                  {phase >= 3 && phase < 5 && (
            ...
            {/* Instruction strip — updates to match current phase */}
            <div className="bg-surface-800 border-t border-white/8 px-4 py-2.5 flex items-center gap-2">
              ...
              <p className="text-[11px] text-zinc-300 font-medium transition-all duration-300">{instruction}</p>
            </div>
          </div>

          <p className="text-[11px] text-zinc-500 leading-relaxed mb-4 text-center">
            This screen appears because Coordie is pending Google's verification. Your calendar data never leaves your device.
          </p>
