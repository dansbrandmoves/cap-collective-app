...
export function AvailabilityRules() {
  const { availabilityRules, productions, slots, calendarEvents, connectedCalendars, prefixRules, slotStates } = useApp()
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-10">
      ...
      <div>
        <p className="text-xs font-semibold text-zinc-600 uppercase tracking-widest mb-3">Project Overrides</p>
        {availabilityRules.length === 0 ? (
          ...
          <div className="space-y-2">
            {availabilityRules.map(rule => {
              ...
              const meta = slotStates[rule.tier]
              return (
                <div key={rule.id} className="bg-surface-900 border border-surface-700 rounded-xl px-5 py-4">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500">{rule.date} · {prod?.name ?? 'Unknown project'}</p>
                    </div>
                    <Badge variant={rule.tier === 'blocked' ? 'red' : rule.tier === 'hold' ? 'yellow' : rule.tier === 'available' ? 'ghost' : 'default'}>
                      {meta?.label ?? rule.tier}
                    </Badge>
                  </div>
                  {rule.privateNote && (
